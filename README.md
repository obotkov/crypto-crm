...
Платформа учёта криптопортфеля. Сейчас в проде — лендинг, страница входа и
макеты экранов приложения. Данных и бизнес-логики пока нет: разметка статична,
переключаются только экраны, табы и шторки.

Экраны собраны по макетам из `crypto-design/` (дизайн-система Nocturne):
навигация — таб-бар снизу до 900px и сайдбар выше, панель сделки — нижняя
шторка на мобильном и правый drawer на десктопе, таблицы на узких экранах
превращаются в списки строк.
...
| Путь | Что это |
| --- | --- |
| `site/index.html` | лендинг — вход в платформу |
| `site/login.html` | страница входа: почта/пароль, Google и шаг с кодом из письма |
| `site/app.html` | экраны приложения: портфели, портфель, монета, монеты, история, настройки |
| `site/assets/app.css` | слой приложения поверх дизайн-системы: раскладка, табы, шторки |
| `site/assets/app.js` | переключение экранов и табов, открытие шторок |
| `site/assets/icons.svg` | спрайт иконок (вместо шрифта с CDN) |
